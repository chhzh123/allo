...
module pe (
  ...
  // Capture register — holds completed sum until drain
  logic signed [31:0] done_reg;
...
module dut_norm (
  ...
  // With registered product, PE(i,j) done_reg written at cycle i+j+9.
  // PE(i,0) at i+9, PE(i,7) at i+16.
  // Load at i+17 captures all (NBA: PE(i,7) old value before overwrite at i+17).
  // Modulo: (i+17) mod 8 = (i+1) mod 8.
  wire load    [0:7];
  wire shift_en [0:7];
  ...
  generate
    for (i = 0; i < 8; i++) begin : row_ctrl
      assign load[i]    = (cycle >= (i + 17)) && (cycle[2:0] == ((i + 1) & 3'd7));
      assign shift_en[i] = (cycle >= (i + 17)) && (cycle[2:0] != ((i + 1) & 3'd7));
    end
  endgenerate
  ...
  always_ff @(posedge ap_clk) begin
    if (!ap_rst_n) begin
      ...
      c_out_write_reg[0] <= (cycle >= 17);
      c_out_write_reg[1] <= (cycle >= 18);
      c_out_write_reg[2] <= (cycle >= 19);
      c_out_write_reg[3] <= (cycle >= 20);
      c_out_write_reg[4] <= (cycle >= 21);
      c_out_write_reg[5] <= (cycle >= 22);
      c_out_write_reg[6] <= (cycle >= 23);
      c_out_write_reg[7] <= (cycle >= 24);
    end
  end
