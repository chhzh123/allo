...
module spmw_harness (
  ...
  wire [63:0] delay0_x_in_bind_dout [0:1];
  wire delay0_x_in_bind_empty_n [0:1];
  wire delay0_x_in_bind_read [0:1];
  genvar g_delay0_x_in_bind;
  generate
    for (g_delay0_x_in_bind = 0; g_delay0_x_in_bind < 2; g_delay0_x_in_bind = g_delay0_x_in_bind + 1) begin : gen_delay0_x_in_bind
  ...
  endgenerate
  wire [63:0] reorder_y_out_bind_din [0:1];
  wire reorder_y_out_bind_write [0:1];
  wire reorder_y_out_bind_full_n [0:1];
  genvar g_reorder_y_out_bind;
  generate
    for (g_reorder_y_out_bind = 0; g_reorder_y_out_bind < 2; g_reorder_y_out_bind = g_reorder_y_out_bind + 1) begin : gen_reorder_y_out_bind
  ...
  always @(posedge ap_clk)
    if (!ap_rst_n) sig <= 32'b0;
      ...
      ^ reorder_y_out_bind_din[1][31:0]
      ^ {31'b0, reorder_y_out_bind_write[1]}
      ;
endmodule
