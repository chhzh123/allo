...
def main():
    ...
    p.add_argument("--loader", choices=("pe", "row"), default="pe", help="weight loader: the published one-PE-a-cycle select, or the row-wise one")
    args = p.parse_args()
    ...
    for name, extra in configs(args.set, args.N):
        ...
        cmd = [sys.executable, RUN, "--rtl", args.rtl, "--N", str(args.N), "--out", run_dir, "--loader", args.loader, *extra]
        t1 = time.time()
