...
BUILDS = "/scratch/hc676/feather_loader/builds"
...
def build(rtl, meta, force=False):
    generics = {k: meta[k] for k in ("N", "T", "MODE", "WA", "IA", "A_BASE", "A_STRIDE", "G", "WLEN")}
    ...
    work = os.path.join(BUILDS, f"{tag}_{meta['loader']}_N{meta['N']}_T{meta['T']}_M{meta['MODE']}_{key}")
...
def main():
    ...
    result.update({"gen_s": t_gen, "build_s": t_build, "sim_s": t_sim, "xsim_rc": rc, "build": work, "rtl": os.path.abspath(args.rtl), "loader": meta["loader"], "WLEN": meta["WLEN"]})
    with open(os.path.join(out, "check.json"), "w", encoding="utf-8") as f:
        json.dump(result, f, indent=1, default=int)
    brief = {k: result.get(k) for k in ("status", "tiles", "tiles_ok", "bad_elements", "pe_files_ok", "pe_files_wrong", "expect_nonzero", "offsets", "first_output_cycles", "completion_cycles", "steady_interval_cycles", "feed_cycles_between_toggles", "toggle_count", "buffers", "host_check", "signed_equals_rtl", "sim_s")}
