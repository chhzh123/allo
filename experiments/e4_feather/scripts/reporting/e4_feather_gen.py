...
def weight_image(files, N):
    """The weight SRAM image for T feeds back to back: rows [T * N^3, N] uint8,
    plus the row the read register must hold when the first feed starts (PE 0's
    first weight of tile 0 in byte 0) -- see the module docstring."""
    T = files.shape[0]
    N2, N3 = N * N, N * N * N
    rows = np.zeros((T, N3, N), dtype=np.uint8)
    a = np.arange(N3)
    q = (a + 1) % N2
    s = (a + 1) // N2
    keep = s < N
    ...
    if T > 1:
        rows[:-1, N3 - 1, 0] = files[1:, 0, 0]
    stale = np.zeros(N, dtype=np.uint8)
    stale[0] = files[0, 0, 0]
    return rows.reshape(T * N3, N), stale
...
def generate(args):
    ...
    # a(t, 0) = G + N^3 for tile 0 (the pass of tile 0 follows feed 0), then
    # strides of N^3 (a feed a tile) or N (passes back to back).
    A_BASE = G + N3
    A_STRIDE = N3 if args.mode == 0 else N
    span = A_BASE + (T - 1) * A_STRIDE + 2 * N + 8 + N3
    IA = max(int(np.ceil(np.log2(span + 1))), 4)
    WA = max(int(np.ceil(np.log2((T if args.mode == 0 else 1) * N3))), 2)
    ...
    if args.mode == 1:
        files = files[:1]
    rows, stale = weight_image(files, N)
    wrow0 = rows[0].copy()
    rows[0] = stale
...
def check(out, xsim_log):
    ...
    result = {
        "status": "pass" if found.all() else "fail",
        "tiles": T,
...
def selftest():
    ...
    for N in (4, 8):
        ...
        rows, stale = weight_image(files, N)
        N2, N3 = N * N, N ** 3
        # feed cycle c: pe_sel = c mod N^2, the read register shows the stale row
        # at c = 0 (address 0 before the start pulse rewrote it) and mem[c-1] after
        ...
        for c in range(T * N3):
            ...
            got[t, q, k] = reg[q // N]
        assert np.array_equal(got, files), "weight image round trip"
    print("selftest ok")
