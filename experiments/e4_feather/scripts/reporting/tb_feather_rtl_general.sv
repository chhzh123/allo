...
//    back to back, N^3 cycles each, addr_end advanced a feed at a time; the
...
module tb;
  ...
  always @(posedge clk)
    if (dbg && F0x > 0 && cyc >= F0x + N3 - 4 && cyc <= F0x + N3 + 3 * N + 12)
  ...
  initial begin
    maxcyc = ((MODE == 0) ? (T + 2) * N3 : 3 * N3 + T * N) + 4000;
    wait (cyc == maxcyc);
