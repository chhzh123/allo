...
module spmw_harness (
  ...
  wire [15:0] pe_x_mem_dout [0:7];
  wire pe_x_mem_empty_n [0:7];
  wire pe_x_mem_read [0:7];
  genvar g_pe_x_mem;
  generate
    for (g_pe_x_mem = 0; g_pe_x_mem < 8; g_pe_x_mem = g_pe_x_mem + 1) begin : gen_pe_x_mem
  ...
  endgenerate
  wire [63:0] pe_w_mem_dout [0:7];
  wire pe_w_mem_empty_n [0:7];
  wire pe_w_mem_read [0:7];
  genvar g_pe_w_mem;
  generate
    for (g_pe_w_mem = 0; g_pe_w_mem < 8; g_pe_w_mem = g_pe_w_mem + 1) begin : gen_pe_w_mem
      ...
      assign pe_w_mem_dout[g_pe_w_mem] = {2{lf_pe_w_mem}};
      assign pe_w_mem_empty_n[g_pe_w_mem] = start;
    end
  endgenerate
  wire [31:0] switch_out_l_bind_din [0:1];
  wire switch_out_l_bind_write [0:1];
  wire switch_out_l_bind_full_n [0:1];
  genvar g_switch_out_l_bind;
  generate
    for (g_switch_out_l_bind = 0; g_switch_out_l_bind < 2; g_switch_out_l_bind = g_switch_out_l_bind + 1) begin : gen_switch_out_l_bind
      assign switch_out_l_bind_full_n[g_switch_out_l_bind] = 1'b1;
    end
  endgenerate
  wire [31:0] switch_out_r_bind_din [0:1];
  wire switch_out_r_bind_write [0:1];
  wire switch_out_r_bind_full_n [0:1];
  genvar g_switch_out_r_bind;
  generate
    for (g_switch_out_r_bind = 0; g_switch_out_r_bind < 2; g_switch_out_r_bind = g_switch_out_r_bind + 1) begin : gen_switch_out_r_bind
      assign switch_out_r_bind_full_n[g_switch_out_r_bind] = 1'b1;
    end
  endgenerate
  wire [15:0] switch_cmd_mem_dout [0:5];
  wire switch_cmd_mem_empty_n [0:5];
  wire switch_cmd_mem_read [0:5];
  genvar g_switch_cmd_mem;
  generate
    for (g_switch_cmd_mem = 0; g_switch_cmd_mem < 6; g_switch_cmd_mem = g_switch_cmd_mem + 1) begin : gen_switch_cmd_mem
