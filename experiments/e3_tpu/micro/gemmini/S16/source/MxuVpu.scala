...
// The MXU-only baseline for differencing. E1 routed `Mesh` alone; this top
// contains `MeshWithDelays`, which wraps Mesh with the skew registers, the tag
// queue and the req/resp control. Differencing MxuVpu against E1's rows would
// charge all of that to the VPU, so the matched baseline is elaborated here
// with exactly the parameters MxuVpu uses.
class MxuOnly(val dim: Int) extends Module {
  val mxu = Module(new MeshWithDelays(SInt(8.W), SInt(8.W), SInt(32.W), SInt(32.W),
    new SimpleTag, Dataflow.WS, tree_reduction = false, tile_latency = 0,
    output_delay = 1, tileRows = 1, tileColumns = 1,
    meshRows = dim, meshColumns = dim, leftBanks = 1, upBanks = 1, outBanks = 1))
  val io = IO(new Bundle {
    ...
    val d = Flipped(Decoupled(chiselTypeOf(mxu.io.d.bits)))
    val req = Flipped(Decoupled(chiselTypeOf(mxu.io.req.bits)))
    val resp = Valid(chiselTypeOf(mxu.io.resp.bits))
  })
  mxu.io.a <> io.a
  mxu.io.b <> io.b
  mxu.io.d <> io.d
  mxu.io.req <> io.req
  io.resp.valid := mxu.io.resp.valid
  io.resp.bits := mxu.io.resp.bits
  dontTouch(mxu.io.tags_in_progress)
}

object ElaborateMxuOnly extends App {
  val dim = sys.env.getOrElse("MESH_DIM", "4").toInt
  println("MXUONLY_ELABORATE_START dim=" + dim)
  val v = (new ChiselStage).emitVerilog(new MxuOnly(dim),
    Array("--target-dir", "mxuonly_out_" + dim))
  println("MXUONLY_ELABORATE_OK dim=" + dim + " verilog_chars=" + v.length)
}
