"""The stage engine as it ran on the U280: the design of record.

A frozen copy of `test_spmw_gpt_stage` at the brick transport (the weight
file streamed as packed words, one `MSWEEP` word per output row, a vector
lane with per-launch constants), the netlist that measured 59.2 ms a layer
at 250 MHz and closed timing at 300 MHz. The test module keeps moving --
`MREP`, lane groups, the fold unit -- and the board results quoted for this
engine should stay rebuildable, so `spmw_build_array.py` offers it as
`gptstage_v1`. Not a test: its tests live in the live module.
"""
