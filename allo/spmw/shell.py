...
def beats_of(fam):
    """How many 512-bit beats one launch of this family occupies.

    The family is a dense stream of ``steps * channels`` tokens, step-major
    and channel-minor, exactly as `host_buffer` writes it; a beat holds
    ``512 / width`` of them, or a token wider than a beat spans ``width / 512``.
    """
    width = fam["width"]
    tokens = fam["steps"] * fam["channels"]
    if width <= BEAT:
        return -(-tokens // (BEAT // width))
    return tokens * (width // BEAT)
...
def feeder_cpp(fam):
    ...
    The AXI side is the family's dense token stream, read a 512-bit beat at a
    time. `host_buffer` lays tokens out step-major and channel-minor as
    contiguous little-endian bytes, so token ``n = t * channels + c`` is bits
    ``[n * width, (n + 1) * width)`` of the stream: a beat holds ``512 / width``
    tokens, a token wider than a beat spans ``width / 512`` of them, and the
    layout is the same one the scalar feeder read one token at a time -- which
    cost about four cycles a token and made a 32,768-step launch DMA-bound by
    fifty times. Reading a beat and dealing out its tokens keeps the feeder as
    fast as the array.
    """
    name = fam["name"]
    width = fam["width"]
    ...
    if token_beats == 1:
        ...
        # Wide tokens: each is `token_beats` consecutive beats.
        if reads:
            body = (
                f"    tok.range(k * {BEAT} + {BEAT - 1}, k * {BEAT}) = "
                f"src[n * {token_beats} + k];\n"
                f"    if (k == {token_beats - 1})\n"
                f"      {port}[n % {channels}].write(tok);"
            )
        else:
            body = (
                f"    if (k == 0)\n"
                f"      tok = {port}[n % {channels}].read();\n"
                f"    dst[n * {token_beats} + k] = "
                f"tok.range(k * {BEAT} + {BEAT - 1}, k * {BEAT});"
            )
        loop = (
            f"  const int total = steps * {channels};\n"
            ...
            f"    for (int k = 0; k < {token_beats}; k++) {{\n"
            f"#pragma HLS pipeline II=1\n"
            f"{body}\n"
            f"    }}\n"
            f"  }}"
        )
    buf = f"const ap_uint<{BEAT}> *src" if reads else f"ap_uint<{BEAT}> *dst"
...
// `{fam['tensor']}`, a dense step-major channel-minor stream read {BEAT} bits
// at a time. {fam['steps']} steps is what this design was elaborated with and
...
_GEOMETRY = {
    "xcu280": {"cols": 8, "rows": 4, "slrs": 3, "dsp_cols": 32, "dsp_rows": 94}
}
...
def dsp_grid_tcl(graph, top="dut", y0=40, part="xcu280"):
    """Pin each mesh cell's DSP to a site so the mesh lies on the DSP grid.

    At 32x32 the placer stopped keeping neighbours near: two cells that share
    a link ended up sixteen DSP rows apart in different columns, and the link
    was a 2.7 ns wire. The mesh's structure is the one thing the placer has
    to rediscover from the netlist, and at this size it did not. This gives
    it the structure: mesh column c on DSP column c (the part has 32), mesh
    row r on DSP row y0 + r, so a partial sum travels one DSP site south and
    an activation one DSP column east. Only the DSP is pinned -- each cell's
    logic and the slices of its links follow it, which is the placer's job
    and the part it does well.

    A Tcl script rather than an XDC: a cell with no DSP, or more than one,
    is skipped rather than an error, and the script is sourced after
    synthesis when the netlist exists.
    """
    geom = _GEOMETRY.get(part.split("-")[0])
    if geom is None:
        raise ValueError(f"no DSP-grid geometry for `{part}`")
    cols_avail = geom.get("dsp_cols", 32)
    emitter = StructuralEmitter(graph)
    lines = [
        "# Generated by allo.spmw.shell.dsp_grid_tcl -- do not edit.",
        "# One DSP site per mesh cell, the mesh laid on the DSP grid.",
        "set spmw_grid_pinned 0",
    ]
    y = y0
    for placement in emitter.placements():
        ...
        if cols > cols_avail:
            raise ValueError(
                f"`{placement.name}` is {cols} columns wide; the part has "
                f"{cols_avail} DSP columns"
            )
        x0 = (cols_avail - cols) // 2
        ...
        for order, (_sig, _rt, sites) in enumerate(emitter.classes(placement)):
            for site in sites:
                ...
                inst = f"{top}/u_{names[order]}_{tag}"
                loc = f"DSP48E2_X{x0 + int(site[1])}Y{y + int(site[0])}"
                lines.append(
                    f"set c [get_cells -quiet -hier -filter "
                    f"{{NAME =~ {inst}/* && REF_NAME == DSP48E2}}]"
                )
                lines.append(
                    f"if {{[llength $c] == 1}} {{ set_property LOC {loc} $c; "
                    f"incr spmw_grid_pinned }}"
                )
        y += rows + 2  # the next mesh, if any, above this one
    lines.append('puts "SPMW DSP GRID pinned $spmw_grid_pinned cell(s)"')
    return "\n".join(lines) + "\n"
