...
Only the solver is here. The MLIR-level rewrite that goes with it
(``apply_f2_layout``, which turns a 1-D memref into a banked 2-D one) lives on
the branch this was taken from; SPMW does not need it, because SPMW rewrites the
access at the Python-AST level before Allo ever sees the buffer -- see
``allo.spmw.bricks.Layout`` and ``allo.spmw.lower_df``.
...
def _rref_gf2(M: np.ndarray) -> tuple:
...
class F2LayoutSolver:
    ...
    def conflict_subspace(self, stride_bits: list) -> np.ndarray:
    ...
    def solve(self, stride_bits: list) -> "SwizzleHelper":
...
def fft_swizzle(N: int, WIDTH: int, stride_bit: int) -> SwizzleHelper:
    ...
    For N=256, WIDTH=32, stride_bit=5, ``helper.swizzle_bank(96)`` gives the
    conflict-free bank for element 96.
    """
