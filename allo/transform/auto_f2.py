...
def symbolize_mlir_index(ssa_value, loop_var_map, n_addr_bits, n_input_bits, memo=None):
    ...
    result = _symbolize_impl(ssa_value, loop_var_map, n_addr_bits, n_input_bits, memo)
    try:
...
def _symbolize_impl(ssa_value, loop_var_map, n_addr_bits, n_input_bits, memo):
    ...
    if op_name == "arith.xori":
        lhs = symbolize_mlir_index(
            op.operands[0], loop_var_map, n_addr_bits, n_input_bits, memo
        )
        rhs = symbolize_mlir_index(
            op.operands[1], loop_var_map, n_addr_bits, n_input_bits, memo
        )
        return lhs.xor(rhs)
    ...
    # arith.index_cast / arith.extui / arith.extsi / arith.trunci → recurse
    if op_name in ("arith.index_cast", "arith.extui", "arith.extsi", "arith.trunci"):
        return symbolize_mlir_index(
            op.operands[0], loop_var_map, n_addr_bits, n_input_bits, memo
        )
    ...
    if op_name == "affine.load":
        ...
        if stored_val is not None:
            return symbolize_mlir_index(
                stored_val, loop_var_map, n_addr_bits, n_input_bits, memo
            )
        return F2Symbol.opaque(n_addr_bits, n_input_bits)
...
def analyze_buffer_conflicts(func_op, alloc_op, n_addr_bits):
    ...
    for acc_type, acc_op, idx_ssa in access_ops:
        if idx_ssa is None:
            ...
            sym = symbolize_mlir_index(
                idx_ssa, loop_var_map, n_addr_bits, n_input_bits, memo
            )
            symbols.append(sym)
...
def auto_apply_f2(module, func_name, func_args, bank_bits=None, kernel_names=None):
    ...
    for fn_name, func_op in target_funcs:
        ...
        if fn_name not in func_args:
            _debug_auto_f2 and print(f"[auto_f2] skip {fn_name}: not in func_args")
            continue
