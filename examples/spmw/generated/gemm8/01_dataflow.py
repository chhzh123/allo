...
@df.region()
def top(A: _T3[4, 4], B: _T3[4, 4], C: int32[4, 4]):
    pe_east_west: Stream[_T3, 2][4, 4]
    pe_south_north: Stream[_T3, 2][4, 4]
    pe_west_bind: Stream[_T3, 2][4]
    pe_north_bind: Stream[_T3, 2][4]

    @df.kernel(mapping=[4], args=[A])
    def pe_west_load(local_A: _T3[4, 4]):
        _q0 = df.get_pid()
        for _t in range(4):
            pe_west_bind[_q0].put(local_A[_q0, _t])

    @df.kernel(mapping=[4], args=[B])
    def pe_north_load(local_B: _T3[4, 4]):
        _q0 = df.get_pid()
        for _t in range(4):
            pe_north_bind[_q0].put(local_B[_t, _q0])

    @df.kernel(mapping=[4, 4], args=[C])
    def pe(local_C: int32[4, 4]):
        _p0, _p1 = df.get_pid()
