`timescale 1ns/1ps

module tb;
  reg clk = 0, rst_n = 0;
  always #5 clk = ~clk;
  integer errors = 0;
  integer produced = 0;
  integer first = -1;
  localparam integer TOTAL = 16;
  wire [7:0] mac_a_in_bind_dout [0:3];
  wire mac_a_in_bind_empty_n [0:3];
  wire mac_a_in_bind_read [0:3];
  reg [7:0] mac_a_in_bind_src0 [0:3];
  integer mac_a_in_bind_p0 = 0;
  initial begin
    mac_a_in_bind_src0[0] = 8'h02;
    mac_a_in_bind_src0[1] = 8'h00;
    mac_a_in_bind_src0[2] = 8'h00;
    mac_a_in_bind_src0[3] = 8'h01;
  end
  assign mac_a_in_bind_dout[0] = mac_a_in_bind_src0[mac_a_in_bind_p0 < 4 ? mac_a_in_bind_p0 : 3];
  assign mac_a_in_bind_empty_n[0] = (mac_a_in_bind_p0 < 4);
  always @(posedge clk) if (rst_n && mac_a_in_bind_read[0] && mac_a_in_bind_empty_n[0]) mac_a_in_bind_p0 <= mac_a_in_bind_p0 + 1;
  reg [7:0] mac_a_in_bind_src1 [0:3];
  integer mac_a_in_bind_p1 = 0;
  initial begin
    mac_a_in_bind_src1[0] = 8'h01;
    mac_a_in_bind_src1[1] = 8'h00;
    mac_a_in_bind_src1[2] = 8'h02;
    mac_a_in_bind_src1[3] = 8'h01;
  end
  assign mac_a_in_bind_dout[1] = mac_a_in_bind_src1[mac_a_in_bind_p1 < 4 ? mac_a_in_bind_p1 : 3];
  assign mac_a_in_bind_empty_n[1] = (mac_a_in_bind_p1 < 4);
  always @(posedge clk) if (rst_n && mac_a_in_bind_read[1] && mac_a_in_bind_empty_n[1]) mac_a_in_bind_p1 <= mac_a_in_bind_p1 + 1;
  reg [7:0] mac_a_in_bind_src2 [0:3];
  integer mac_a_in_bind_p2 = 0;
  initial begin
    mac_a_in_bind_src2[0] = 8'h01;
    mac_a_in_bind_src2[1] = 8'h00;
    mac_a_in_bind_src2[2] = 8'h01;
    mac_a_in_bind_src2[3] = 8'h02;
  end
  assign mac_a_in_bind_dout[2] = mac_a_in_bind_src2[mac_a_in_bind_p2 < 4 ? mac_a_in_bind_p2 : 3];
  assign mac_a_in_bind_empty_n[2] = (mac_a_in_bind_p2 < 4);
  always @(posedge clk) if (rst_n && mac_a_in_bind_read[2] && mac_a_in_bind_empty_n[2]) mac_a_in_bind_p2 <= mac_a_in_bind_p2 + 1;
  reg [7:0] mac_a_in_bind_src3 [0:3];
  integer mac_a_in_bind_p3 = 0;
  initial begin
    mac_a_in_bind_src3[0] = 8'h00;
    mac_a_in_bind_src3[1] = 8'h00;
    mac_a_in_bind_src3[2] = 8'h02;
    mac_a_in_bind_src3[3] = 8'h02;
  end
  assign mac_a_in_bind_dout[3] = mac_a_in_bind_src3[mac_a_in_bind_p3 < 4 ? mac_a_in_bind_p3 : 3];
  assign mac_a_in_bind_empty_n[3] = (mac_a_in_bind_p3 < 4);
  always @(posedge clk) if (rst_n && mac_a_in_bind_read[3] && mac_a_in_bind_empty_n[3]) mac_a_in_bind_p3 <= mac_a_in_bind_p3 + 1;
  wire [31:0] mac_op_in_bind_dout [0:3];
  wire mac_op_in_bind_empty_n [0:3];
  wire mac_op_in_bind_read [0:3];
  reg [31:0] mac_op_in_bind_src0 [0:3];
  integer mac_op_in_bind_p0 = 0;
  initial begin
    mac_op_in_bind_src0[0] = 32'h01010000;
    mac_op_in_bind_src0[1] = 32'h01010000;
    mac_op_in_bind_src0[2] = 32'h01010000;
    mac_op_in_bind_src0[3] = 32'h01010000;
  end
  assign mac_op_in_bind_dout[0] = mac_op_in_bind_src0[mac_op_in_bind_p0 < 4 ? mac_op_in_bind_p0 : 3];
  assign mac_op_in_bind_empty_n[0] = (mac_op_in_bind_p0 < 4);
  always @(posedge clk) if (rst_n && mac_op_in_bind_read[0] && mac_op_in_bind_empty_n[0]) mac_op_in_bind_p0 <= mac_op_in_bind_p0 + 1;
  reg [31:0] mac_op_in_bind_src1 [0:3];
  integer mac_op_in_bind_p1 = 0;
  initial begin
    mac_op_in_bind_src1[0] = 32'h01010000;
    mac_op_in_bind_src1[1] = 32'h01010000;
    mac_op_in_bind_src1[2] = 32'h01010000;
    mac_op_in_bind_src1[3] = 32'h01010000;
  end
  assign mac_op_in_bind_dout[1] = mac_op_in_bind_src1[mac_op_in_bind_p1 < 4 ? mac_op_in_bind_p1 : 3];
  assign mac_op_in_bind_empty_n[1] = (mac_op_in_bind_p1 < 4);
  always @(posedge clk) if (rst_n && mac_op_in_bind_read[1] && mac_op_in_bind_empty_n[1]) mac_op_in_bind_p1 <= mac_op_in_bind_p1 + 1;
  reg [31:0] mac_op_in_bind_src2 [0:3];
  integer mac_op_in_bind_p2 = 0;
  initial begin
    mac_op_in_bind_src2[0] = 32'h01010000;
    mac_op_in_bind_src2[1] = 32'h01010000;
    mac_op_in_bind_src2[2] = 32'h01010000;
    mac_op_in_bind_src2[3] = 32'h01010000;
  end
  assign mac_op_in_bind_dout[2] = mac_op_in_bind_src2[mac_op_in_bind_p2 < 4 ? mac_op_in_bind_p2 : 3];
  assign mac_op_in_bind_empty_n[2] = (mac_op_in_bind_p2 < 4);
  always @(posedge clk) if (rst_n && mac_op_in_bind_read[2] && mac_op_in_bind_empty_n[2]) mac_op_in_bind_p2 <= mac_op_in_bind_p2 + 1;
  reg [31:0] mac_op_in_bind_src3 [0:3];
  integer mac_op_in_bind_p3 = 0;
  initial begin
    mac_op_in_bind_src3[0] = 32'h01010000;
    mac_op_in_bind_src3[1] = 32'h01010000;
    mac_op_in_bind_src3[2] = 32'h01010000;
    mac_op_in_bind_src3[3] = 32'h01010000;
  end
  assign mac_op_in_bind_dout[3] = mac_op_in_bind_src3[mac_op_in_bind_p3 < 4 ? mac_op_in_bind_p3 : 3];
  assign mac_op_in_bind_empty_n[3] = (mac_op_in_bind_p3 < 4);
  always @(posedge clk) if (rst_n && mac_op_in_bind_read[3] && mac_op_in_bind_empty_n[3]) mac_op_in_bind_p3 <= mac_op_in_bind_p3 + 1;
  wire [31:0] mac_w_mem_dout [0:15];
  wire mac_w_mem_empty_n [0:15];
  wire mac_w_mem_read [0:15];
  reg [31:0] mac_w_mem_src0 [0:0];
  integer mac_w_mem_p0 = 0;
  initial begin
    mac_w_mem_src0[0] = 32'h02010101;
  end
  assign mac_w_mem_dout[0] = mac_w_mem_src0[mac_w_mem_p0 < 1 ? mac_w_mem_p0 : 0];
  assign mac_w_mem_empty_n[0] = (mac_w_mem_p0 < 1);
  always @(posedge clk) if (rst_n && mac_w_mem_read[0] && mac_w_mem_empty_n[0]) mac_w_mem_p0 <= mac_w_mem_p0 + 1;
  reg [31:0] mac_w_mem_src1 [0:0];
  integer mac_w_mem_p1 = 0;
  initial begin
    mac_w_mem_src1[0] = 32'h00020200;
  end
  assign mac_w_mem_dout[1] = mac_w_mem_src1[mac_w_mem_p1 < 1 ? mac_w_mem_p1 : 0];
  assign mac_w_mem_empty_n[1] = (mac_w_mem_p1 < 1);
  always @(posedge clk) if (rst_n && mac_w_mem_read[1] && mac_w_mem_empty_n[1]) mac_w_mem_p1 <= mac_w_mem_p1 + 1;
  reg [31:0] mac_w_mem_src2 [0:0];
  integer mac_w_mem_p2 = 0;
  initial begin
    mac_w_mem_src2[0] = 32'h00010201;
  end
  assign mac_w_mem_dout[2] = mac_w_mem_src2[mac_w_mem_p2 < 1 ? mac_w_mem_p2 : 0];
  assign mac_w_mem_empty_n[2] = (mac_w_mem_p2 < 1);
  always @(posedge clk) if (rst_n && mac_w_mem_read[2] && mac_w_mem_empty_n[2]) mac_w_mem_p2 <= mac_w_mem_p2 + 1;
  reg [31:0] mac_w_mem_src3 [0:0];
  integer mac_w_mem_p3 = 0;
  initial begin
    mac_w_mem_src3[0] = 32'h00020202;
  end
  assign mac_w_mem_dout[3] = mac_w_mem_src3[mac_w_mem_p3 < 1 ? mac_w_mem_p3 : 0];
  assign mac_w_mem_empty_n[3] = (mac_w_mem_p3 < 1);
  always @(posedge clk) if (rst_n && mac_w_mem_read[3] && mac_w_mem_empty_n[3]) mac_w_mem_p3 <= mac_w_mem_p3 + 1;
  reg [31:0] mac_w_mem_src4 [0:0];
  integer mac_w_mem_p4 = 0;
  initial begin
    mac_w_mem_src4[0] = 32'h01000200;
  end
  assign mac_w_mem_dout[4] = mac_w_mem_src4[mac_w_mem_p4 < 1 ? mac_w_mem_p4 : 0];
  assign mac_w_mem_empty_n[4] = (mac_w_mem_p4 < 1);
  always @(posedge clk) if (rst_n && mac_w_mem_read[4] && mac_w_mem_empty_n[4]) mac_w_mem_p4 <= mac_w_mem_p4 + 1;
  reg [31:0] mac_w_mem_src5 [0:0];
  integer mac_w_mem_p5 = 0;
  initial begin
    mac_w_mem_src5[0] = 32'h01010000;
  end
  assign mac_w_mem_dout[5] = mac_w_mem_src5[mac_w_mem_p5 < 1 ? mac_w_mem_p5 : 0];
  assign mac_w_mem_empty_n[5] = (mac_w_mem_p5 < 1);
  always @(posedge clk) if (rst_n && mac_w_mem_read[5] && mac_w_mem_empty_n[5]) mac_w_mem_p5 <= mac_w_mem_p5 + 1;
  reg [31:0] mac_w_mem_src6 [0:0];
  integer mac_w_mem_p6 = 0;
  initial begin
    mac_w_mem_src6[0] = 32'h00000001;
  end
  assign mac_w_mem_dout[6] = mac_w_mem_src6[mac_w_mem_p6 < 1 ? mac_w_mem_p6 : 0];
  assign mac_w_mem_empty_n[6] = (mac_w_mem_p6 < 1);
  always @(posedge clk) if (rst_n && mac_w_mem_read[6] && mac_w_mem_empty_n[6]) mac_w_mem_p6 <= mac_w_mem_p6 + 1;
  reg [31:0] mac_w_mem_src7 [0:0];
  integer mac_w_mem_p7 = 0;
  initial begin
    mac_w_mem_src7[0] = 32'h01010200;
  end
  assign mac_w_mem_dout[7] = mac_w_mem_src7[mac_w_mem_p7 < 1 ? mac_w_mem_p7 : 0];
  assign mac_w_mem_empty_n[7] = (mac_w_mem_p7 < 1);
  always @(posedge clk) if (rst_n && mac_w_mem_read[7] && mac_w_mem_empty_n[7]) mac_w_mem_p7 <= mac_w_mem_p7 + 1;
  reg [31:0] mac_w_mem_src8 [0:0];
  integer mac_w_mem_p8 = 0;
  initial begin
    mac_w_mem_src8[0] = 32'h01020100;
  end
  assign mac_w_mem_dout[8] = mac_w_mem_src8[mac_w_mem_p8 < 1 ? mac_w_mem_p8 : 0];
  assign mac_w_mem_empty_n[8] = (mac_w_mem_p8 < 1);
  always @(posedge clk) if (rst_n && mac_w_mem_read[8] && mac_w_mem_empty_n[8]) mac_w_mem_p8 <= mac_w_mem_p8 + 1;
  reg [31:0] mac_w_mem_src9 [0:0];
  integer mac_w_mem_p9 = 0;
  initial begin
    mac_w_mem_src9[0] = 32'h02020201;
  end
  assign mac_w_mem_dout[9] = mac_w_mem_src9[mac_w_mem_p9 < 1 ? mac_w_mem_p9 : 0];
  assign mac_w_mem_empty_n[9] = (mac_w_mem_p9 < 1);
  always @(posedge clk) if (rst_n && mac_w_mem_read[9] && mac_w_mem_empty_n[9]) mac_w_mem_p9 <= mac_w_mem_p9 + 1;
  reg [31:0] mac_w_mem_src10 [0:0];
  integer mac_w_mem_p10 = 0;
  initial begin
    mac_w_mem_src10[0] = 32'h01020201;
  end
  assign mac_w_mem_dout[10] = mac_w_mem_src10[mac_w_mem_p10 < 1 ? mac_w_mem_p10 : 0];
  assign mac_w_mem_empty_n[10] = (mac_w_mem_p10 < 1);
  always @(posedge clk) if (rst_n && mac_w_mem_read[10] && mac_w_mem_empty_n[10]) mac_w_mem_p10 <= mac_w_mem_p10 + 1;
  reg [31:0] mac_w_mem_src11 [0:0];
  integer mac_w_mem_p11 = 0;
  initial begin
    mac_w_mem_src11[0] = 32'h01020202;
  end
  assign mac_w_mem_dout[11] = mac_w_mem_src11[mac_w_mem_p11 < 1 ? mac_w_mem_p11 : 0];
  assign mac_w_mem_empty_n[11] = (mac_w_mem_p11 < 1);
  always @(posedge clk) if (rst_n && mac_w_mem_read[11] && mac_w_mem_empty_n[11]) mac_w_mem_p11 <= mac_w_mem_p11 + 1;
  reg [31:0] mac_w_mem_src12 [0:0];
  integer mac_w_mem_p12 = 0;
  initial begin
    mac_w_mem_src12[0] = 32'h02010002;
  end
  assign mac_w_mem_dout[12] = mac_w_mem_src12[mac_w_mem_p12 < 1 ? mac_w_mem_p12 : 0];
  assign mac_w_mem_empty_n[12] = (mac_w_mem_p12 < 1);
  always @(posedge clk) if (rst_n && mac_w_mem_read[12] && mac_w_mem_empty_n[12]) mac_w_mem_p12 <= mac_w_mem_p12 + 1;
  reg [31:0] mac_w_mem_src13 [0:0];
  integer mac_w_mem_p13 = 0;
  initial begin
    mac_w_mem_src13[0] = 32'h00010102;
  end
  assign mac_w_mem_dout[13] = mac_w_mem_src13[mac_w_mem_p13 < 1 ? mac_w_mem_p13 : 0];
  assign mac_w_mem_empty_n[13] = (mac_w_mem_p13 < 1);
  always @(posedge clk) if (rst_n && mac_w_mem_read[13] && mac_w_mem_empty_n[13]) mac_w_mem_p13 <= mac_w_mem_p13 + 1;
  reg [31:0] mac_w_mem_src14 [0:0];
  integer mac_w_mem_p14 = 0;
  initial begin
    mac_w_mem_src14[0] = 32'h02020101;
  end
  assign mac_w_mem_dout[14] = mac_w_mem_src14[mac_w_mem_p14 < 1 ? mac_w_mem_p14 : 0];
  assign mac_w_mem_empty_n[14] = (mac_w_mem_p14 < 1);
  always @(posedge clk) if (rst_n && mac_w_mem_read[14] && mac_w_mem_empty_n[14]) mac_w_mem_p14 <= mac_w_mem_p14 + 1;
  reg [31:0] mac_w_mem_src15 [0:0];
  integer mac_w_mem_p15 = 0;
  initial begin
    mac_w_mem_src15[0] = 32'h01010200;
  end
  assign mac_w_mem_dout[15] = mac_w_mem_src15[mac_w_mem_p15 < 1 ? mac_w_mem_p15 : 0];
  assign mac_w_mem_empty_n[15] = (mac_w_mem_p15 < 1);
  always @(posedge clk) if (rst_n && mac_w_mem_read[15] && mac_w_mem_empty_n[15]) mac_w_mem_p15 <= mac_w_mem_p15 + 1;
  wire [31:0] vpu_op_in_bind_dout [0:0];
  wire vpu_op_in_bind_empty_n [0:0];
  wire vpu_op_in_bind_read [0:0];
  reg [31:0] vpu_op_in_bind_src0 [0:15];
  integer vpu_op_in_bind_p0 = 0;
  initial begin
    vpu_op_in_bind_src0[0] = 32'h03000000;
    vpu_op_in_bind_src0[1] = 32'h09000000;
    vpu_op_in_bind_src0[2] = 32'h08000000;
    vpu_op_in_bind_src0[3] = 32'h00000000;
    vpu_op_in_bind_src0[4] = 32'h00000000;
    vpu_op_in_bind_src0[5] = 32'h00000000;
    vpu_op_in_bind_src0[6] = 32'h00000000;
    vpu_op_in_bind_src0[7] = 32'h00000000;
    vpu_op_in_bind_src0[8] = 32'h00000000;
    vpu_op_in_bind_src0[9] = 32'h00000000;
    vpu_op_in_bind_src0[10] = 32'h00000000;
    vpu_op_in_bind_src0[11] = 32'h00000000;
    vpu_op_in_bind_src0[12] = 32'h00000000;
    vpu_op_in_bind_src0[13] = 32'h00000000;
    vpu_op_in_bind_src0[14] = 32'h00000000;
    vpu_op_in_bind_src0[15] = 32'h00000000;
  end
  assign vpu_op_in_bind_dout[0] = vpu_op_in_bind_src0[vpu_op_in_bind_p0 < 16 ? vpu_op_in_bind_p0 : 15];
  assign vpu_op_in_bind_empty_n[0] = (vpu_op_in_bind_p0 < 16);
  always @(posedge clk) if (rst_n && vpu_op_in_bind_read[0] && vpu_op_in_bind_empty_n[0]) vpu_op_in_bind_p0 <= vpu_op_in_bind_p0 + 1;
  wire [63:0] vpu_b_mem_dout [0:3];
  wire vpu_b_mem_empty_n [0:3];
  wire vpu_b_mem_read [0:3];
  reg [63:0] vpu_b_mem_src0 [0:0];
  integer vpu_b_mem_p0 = 0;
  initial begin
    vpu_b_mem_src0[0] = 64'h0000000100000002;
  end
  assign vpu_b_mem_dout[0] = vpu_b_mem_src0[vpu_b_mem_p0 < 1 ? vpu_b_mem_p0 : 0];
  assign vpu_b_mem_empty_n[0] = (vpu_b_mem_p0 < 1);
  always @(posedge clk) if (rst_n && vpu_b_mem_read[0] && vpu_b_mem_empty_n[0]) vpu_b_mem_p0 <= vpu_b_mem_p0 + 1;
  reg [63:0] vpu_b_mem_src1 [0:0];
  integer vpu_b_mem_p1 = 0;
  initial begin
    vpu_b_mem_src1[0] = 64'h0000000000000000;
  end
  assign vpu_b_mem_dout[1] = vpu_b_mem_src1[vpu_b_mem_p1 < 1 ? vpu_b_mem_p1 : 0];
  assign vpu_b_mem_empty_n[1] = (vpu_b_mem_p1 < 1);
  always @(posedge clk) if (rst_n && vpu_b_mem_read[1] && vpu_b_mem_empty_n[1]) vpu_b_mem_p1 <= vpu_b_mem_p1 + 1;
  reg [63:0] vpu_b_mem_src2 [0:0];
  integer vpu_b_mem_p2 = 0;
  initial begin
    vpu_b_mem_src2[0] = 64'h0000000100000002;
  end
  assign vpu_b_mem_dout[2] = vpu_b_mem_src2[vpu_b_mem_p2 < 1 ? vpu_b_mem_p2 : 0];
  assign vpu_b_mem_empty_n[2] = (vpu_b_mem_p2 < 1);
  always @(posedge clk) if (rst_n && vpu_b_mem_read[2] && vpu_b_mem_empty_n[2]) vpu_b_mem_p2 <= vpu_b_mem_p2 + 1;
  reg [63:0] vpu_b_mem_src3 [0:0];
  integer vpu_b_mem_p3 = 0;
  initial begin
    vpu_b_mem_src3[0] = 64'h0000000100000001;
  end
  assign vpu_b_mem_dout[3] = vpu_b_mem_src3[vpu_b_mem_p3 < 1 ? vpu_b_mem_p3 : 0];
  assign vpu_b_mem_empty_n[3] = (vpu_b_mem_p3 < 1);
  always @(posedge clk) if (rst_n && vpu_b_mem_read[3] && vpu_b_mem_empty_n[3]) vpu_b_mem_p3 <= vpu_b_mem_p3 + 1;
  wire [31:0] vpu_y_out_bind_din [0:3];
  wire vpu_y_out_bind_write [0:3];
  wire vpu_y_out_bind_full_n [0:3];
  reg [31:0] vpu_y_out_bind_exp0 [0:3];
  integer vpu_y_out_bind_q0 = 0;
  initial begin
    vpu_y_out_bind_exp0[0] = 32'h00000005;
    vpu_y_out_bind_exp0[1] = 32'h00000000;
    vpu_y_out_bind_exp0[2] = 32'h00000005;
    vpu_y_out_bind_exp0[3] = 32'h00000005;
  end
  assign vpu_y_out_bind_full_n[0] = 1'b1;
  always @(posedge clk) if (rst_n && vpu_y_out_bind_write[0]) begin
    if (vpu_y_out_bind_q0 < 4) begin
      if (vpu_y_out_bind_din[0] !== vpu_y_out_bind_exp0[vpu_y_out_bind_q0]) begin
        errors = errors + 1;
        $display("MISMATCH vpu_y_out_bind[%0d] step %0d: got %h want %h",
                 0, vpu_y_out_bind_q0, vpu_y_out_bind_din[0], vpu_y_out_bind_exp0[vpu_y_out_bind_q0]);
      end
      vpu_y_out_bind_q0 <= vpu_y_out_bind_q0 + 1;
      produced = produced + 1;
    end else begin
      errors = errors + 1;
      $display("EXTRA TOKEN on vpu_y_out_bind[%0d]", 0);
    end
  end
  reg [31:0] vpu_y_out_bind_exp1 [0:3];
  integer vpu_y_out_bind_q1 = 0;
  initial begin
    vpu_y_out_bind_exp1[0] = 32'h00000006;
    vpu_y_out_bind_exp1[1] = 32'h00000000;
    vpu_y_out_bind_exp1[2] = 32'h00000004;
    vpu_y_out_bind_exp1[3] = 32'h00000008;
  end
  assign vpu_y_out_bind_full_n[1] = 1'b1;
  always @(posedge clk) if (rst_n && vpu_y_out_bind_write[1]) begin
    if (vpu_y_out_bind_q1 < 4) begin
      if (vpu_y_out_bind_din[1] !== vpu_y_out_bind_exp1[vpu_y_out_bind_q1]) begin
        errors = errors + 1;
        $display("MISMATCH vpu_y_out_bind[%0d] step %0d: got %h want %h",
                 1, vpu_y_out_bind_q1, vpu_y_out_bind_din[1], vpu_y_out_bind_exp1[vpu_y_out_bind_q1]);
      end
      vpu_y_out_bind_q1 <= vpu_y_out_bind_q1 + 1;
      produced = produced + 1;
    end else begin
      errors = errors + 1;
      $display("EXTRA TOKEN on vpu_y_out_bind[%0d]", 1);
    end
  end
  reg [31:0] vpu_y_out_bind_exp2 [0:3];
  integer vpu_y_out_bind_q2 = 0;
  initial begin
    vpu_y_out_bind_exp2[0] = 32'h00000006;
    vpu_y_out_bind_exp2[1] = 32'h00000000;
    vpu_y_out_bind_exp2[2] = 32'h00000004;
    vpu_y_out_bind_exp2[3] = 32'h00000008;
  end
  assign vpu_y_out_bind_full_n[2] = 1'b1;
  always @(posedge clk) if (rst_n && vpu_y_out_bind_write[2]) begin
    if (vpu_y_out_bind_q2 < 4) begin
      if (vpu_y_out_bind_din[2] !== vpu_y_out_bind_exp2[vpu_y_out_bind_q2]) begin
        errors = errors + 1;
        $display("MISMATCH vpu_y_out_bind[%0d] step %0d: got %h want %h",
                 2, vpu_y_out_bind_q2, vpu_y_out_bind_din[2], vpu_y_out_bind_exp2[vpu_y_out_bind_q2]);
      end
      vpu_y_out_bind_q2 <= vpu_y_out_bind_q2 + 1;
      produced = produced + 1;
    end else begin
      errors = errors + 1;
      $display("EXTRA TOKEN on vpu_y_out_bind[%0d]", 2);
    end
  end
  reg [31:0] vpu_y_out_bind_exp3 [0:3];
  integer vpu_y_out_bind_q3 = 0;
  initial begin
    vpu_y_out_bind_exp3[0] = 32'h00000008;
    vpu_y_out_bind_exp3[1] = 32'h00000000;
    vpu_y_out_bind_exp3[2] = 32'h0000000a;
    vpu_y_out_bind_exp3[3] = 32'h0000000c;
  end
  assign vpu_y_out_bind_full_n[3] = 1'b1;
  always @(posedge clk) if (rst_n && vpu_y_out_bind_write[3]) begin
    if (vpu_y_out_bind_q3 < 4) begin
      if (vpu_y_out_bind_din[3] !== vpu_y_out_bind_exp3[vpu_y_out_bind_q3]) begin
        errors = errors + 1;
        $display("MISMATCH vpu_y_out_bind[%0d] step %0d: got %h want %h",
                 3, vpu_y_out_bind_q3, vpu_y_out_bind_din[3], vpu_y_out_bind_exp3[vpu_y_out_bind_q3]);
      end
      vpu_y_out_bind_q3 <= vpu_y_out_bind_q3 + 1;
      produced = produced + 1;
    end else begin
      errors = errors + 1;
      $display("EXTRA TOKEN on vpu_y_out_bind[%0d]", 3);
    end
  end
  spmw_top dut (.ap_clk(clk), .ap_rst_n(rst_n), .mac_a_in_bind_dout(mac_a_in_bind_dout), .mac_a_in_bind_empty_n(mac_a_in_bind_empty_n), .mac_a_in_bind_read(mac_a_in_bind_read), .mac_op_in_bind_dout(mac_op_in_bind_dout), .mac_op_in_bind_empty_n(mac_op_in_bind_empty_n), .mac_op_in_bind_read(mac_op_in_bind_read), .mac_w_mem_dout(mac_w_mem_dout), .mac_w_mem_empty_n(mac_w_mem_empty_n), .mac_w_mem_read(mac_w_mem_read), .vpu_op_in_bind_dout(vpu_op_in_bind_dout), .vpu_op_in_bind_empty_n(vpu_op_in_bind_empty_n), .vpu_op_in_bind_read(vpu_op_in_bind_read), .vpu_b_mem_dout(vpu_b_mem_dout), .vpu_b_mem_empty_n(vpu_b_mem_empty_n), .vpu_b_mem_read(vpu_b_mem_read), .vpu_y_out_bind_din(vpu_y_out_bind_din), .vpu_y_out_bind_write(vpu_y_out_bind_write), .vpu_y_out_bind_full_n(vpu_y_out_bind_full_n));
  initial begin
    repeat (4) @(posedge clk);
    rst_n = 1;
    for (integer c = 0; c < 200000; c = c + 1) begin
      @(posedge clk);
      if (produced > 0 && first < 0) first = c;
      if (produced == TOTAL) begin
        $display("SPMW COSIM %s (%0d/%0d tokens, %0d errors)",
                 errors == 0 ? "PASS" : "FAIL", produced, TOTAL, errors);
        $display("SPMW CYCLES total=%0d first_out=%0d",
                 c + 1, first + 1);
        $finish;
      end
    end
    $display("SPMW COSIM TIMEOUT (%0d/%0d tokens, %0d errors)",
             produced, TOTAL, errors);
    $display("SPMW CYCLES total=-1 first_out=%0d", first + 1);
    $finish;
  end
endmodule
