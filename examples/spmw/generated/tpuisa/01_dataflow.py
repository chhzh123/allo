...
@df.region()
def top(A: _T21[4, 4], W: _T21[4, 4, 4], Bias: int32[4, 2], MProg: int32[4, 4], VProg: int32[16], Y: int32[4, 4]):
    ...
    def mac_a_in_load(local_A: _T21[4, 4]):
        _q0 = df.get_pid()
        for _t in range(4):
            mac_a_in_bind[_q0].put(local_A[_t, _q0])

    @df.kernel(mapping=[4], args=[MProg])
    def mac_op_in_load(local_MProg: int32[4, 4]):
        _q0 = df.get_pid()
        for _t in range(4):
